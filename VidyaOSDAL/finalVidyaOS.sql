...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM Schools WHERE SchoolCode = @SchoolCode)
        THROW 50001, 'School code already exists', 1;

    -- Admin username must be globally unique
    IF EXISTS (SELECT 1 FROM Users WHERE Username = @AdminUsername)
        THROW 50002, 'Admin username already exists', 1;
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM AdmissionYearSequences
        WHERE SchoolId = @SchoolId AND AdmissionYear = @AdmissionYear
    )
    BEGIN
        INSERT INTO AdmissionYearSequences
        VALUES (@SchoolId, @AdmissionYear, 0);
    END

    UPDATE AdmissionYearSequences
    SET LastSeq = LastSeq + 1
    WHERE SchoolId = @SchoolId AND AdmissionYear = @AdmissionYear;

    DECLARE @Seq INT;
    SELECT @Seq = LastSeq
    FROM AdmissionYearSequences
    WHERE SchoolId = @SchoolId AND AdmissionYear = @AdmissionYear;

    SET @AdmissionNo =
        CONCAT(@SchoolPrefix, '-', @AdmissionYear, '-', FORMAT(@Seq, '0000'));
